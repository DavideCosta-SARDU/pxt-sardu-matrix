namespace sarduMatrix {
    // Encoded SARDU Matrix Graphic format:
    // SMG1, width, height, encoding, palette count, RGB palette, pixel payload.
    ...
    /** Identity used by the block editor for generated graphic instances. */
    ...
    export function graphicWidth(graphic: MatrixGraphic): number {
        const data = graphic ? graphic._data() : null;
        return validGraphic(data) ? data[4] : 0;
    }

    export function graphicHeight(graphic: MatrixGraphic): number {
        const data = graphic ? graphic._data() : null;
        return validGraphic(data) ? data[5] : 0;
    }
    ...
    export function drawGraphic(
        ...
        x = Math.floor(x);
        y = Math.floor(y);
        mode = mode == MatrixGraphicMode.ReplaceArea ? MatrixGraphicMode.ReplaceArea : MatrixGraphicMode.Overlay;
        const width = data[4];
        const height = data[5];
        const colors = data[7];
        for (let gy = 0; gy < height; gy++) {
            for (let gx = 0; gx < width; gx++) {
                ...
                if (index == 0) {
                    if (mode == MatrixGraphicMode.ReplaceArea) matrix.setPixel(x + gx, y + gy, 0);
