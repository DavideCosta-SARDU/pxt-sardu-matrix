...
function encode(graphic: Graphic): string {
  const colors: string[] = []
  for (const pixel of graphic.pixels) {
    if (pixel && !colors.includes(pixel)) colors.push(pixel)
  }
  ...
  if (mask) {
    for (let start = 0; start < graphic.pixels.length; start += 8) {
      ...
      for (let bit = 0; bit < 8; bit++)
        if (graphic.pixels[start + bit]) packed |= 1 << (7 - bit)
      out.push(packed)
    }
  } else {
...
function generatedCode(): string {
  ...
  for (const [id, graphic] of Object.entries(store.graphics)) {
    ...
    code += `    export const ${identifier(id)} = sarduMatrix.graphicFromBuffer(hex\`${encode(graphic)}\`)\n\n`
  }
  return code + "}\n"
}
...
function post(action: string, body?: ExtensionMessage["body"]) {
  ...
  const message = { type: "pxtpkgext", action, extId, response: true, id, body }
  window.parent.postMessage(message, "*")
}

function save() {
  const graphic = current()
  const nameInput = document.querySelector<HTMLInputElement>("#graphic-name")!
  if (graphic) graphic.name = nameInput.value.trim() || "Grafica"
  const json = JSON.stringify(store, null, 2)
  localStorage.setItem("sardu-matrix-graphics", json)
  const code = generatedCode()
  if (extId) post("extwritecode", { code, json })
  const output = document.querySelector<HTMLTextAreaElement>("#generated-code")!
  output.value = code
  setStatus(extId ? "Grafica salvata nel progetto MakeCode." : "Grafica salvata nel browser; copia il codice generato nel progetto MakeCode.")
  renderAssetOptions()
}
...
function render() {
  if (!currentId || !store.graphics[currentId]) {
    addGraphic()
    return
  }
  const graphic = current()!
  app.innerHTML = `
    <section class="shell">
      ...
      <p class="subtitle">Disegna immagini compatte per tutti i formati supportati. Trasparente e nero sono due strumenti distinti.</p>
      <div class="toolbar">
      ...
      <div class="workspace">
        <aside class="panel">
          ...
          <p class="hint"><b>Trasparente</b> lascia invariato ciò che è già disegnato in modalità sovrapponi. <b>Nero</b> è un colore reale e spegne il LED. Sono disponibili al massimo 15 colori per grafica.</p>
        </aside>
        <section class="panel">
          <h2>Disegno ${graphic.width} × ${graphic.height}</h2>
          <div class="grid-wrap"><div id="pixel-grid" class="pixel-grid"></div></div>
        </section>
      </div>
...
window.addEventListener("message", event => {
  ...
  if (action === "extinit") {
    ...
    } catch { setStatus("I dati grafici esistenti non sono leggibili; è stato creato un nuovo progetto grafico.") }
    currentId = Object.keys(store.graphics || {})[0] || ""
...
if (cached) {
  try { store = JSON.parse(cached) as Store } catch { /* start empty */ }
}
currentId = Object.keys(store.graphics || {})[0] || ""
if (!currentId) addGraphic(); else render()
