...
type MatrixMessage = FrameMessage | InitMessage
...
function applyFrame(message: FrameMessage) {
  ...
  for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
    ...
    // NeoPixel RGB buffers store bytes in GRB order.
...
function receiveMatrixMessage(message: MatrixMessage) {
  if (message.type === "init") {
    ...
    render()
  } else if (message.type === "frame") applyFrame(message)
}
...
window.addEventListener("message", event => {
  ...
  } catch { /* Ignore malformed simulator packets. */ }
})
